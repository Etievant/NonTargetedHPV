...
# We consider the case where these observed covariate only has a few large strata.
...
Onerun = function(p){
  ...
  # Mean of the primary outcome, E(Y1^(16))
  meanY1 = pY1[1]
  
  # Variance of the primary outcome
  ...
  # Covariance between primary and secondary outcomes
  covY1Y2s_part1 = 0
  for(i in 1:length(wRegion)){
    for(j in 1:length(wAge)){
      for(p in 1:length(a)){
        ...
        covY1Y2s_part1 = covY1Y2s_part1 + t(exp(mu_2 + wAge[j] %*% t(s_WAge) + wRegion[i] %*% t(s_WRegion))) %*% (exp(mu_1[1] + wAge[j] %*% t(q_WAge[1]) + t(q_WRegion[i,1])) * (t(exp(beta_1[1])) * pt + 1 - pt)) * a[p]^2 * (p == 1) * Palow[i,j] * PwAge[i,j] * PwRegion[i]
        covY1Y2s_part1 = covY1Y2s_part1 + t(exp(mu_2 + wAge[j] %*% t(s_WAge) + wRegion[i] %*% t(s_WRegion))) %*% (exp(mu_1[1] + wAge[j] %*% t(q_WAge[1]) + t(q_WRegion[i,1])) * (t(exp(beta_1[1])) * pt + 1 - pt)) * a[p]^2 * (p == 2) * Pamedium[i,j] * PwAge[i,j] * PwRegion[i]
        covY1Y2s_part1 = covY1Y2s_part1 + t(exp(mu_2 + wAge[j] %*% t(s_WAge) + wRegion[i] %*% t(s_WRegion))) %*% (exp(mu_1[1] + wAge[j] %*% t(q_WAge[1]) + t(q_WRegion[i,1])) * (t(exp(beta_1[1])) * pt + 1 - pt)) * a[p]^2 * (p == 3) * Pahigh[i,j] * PwAge[i,j] * PwRegion[i]
      }}}
  covY1Y2s = sum(covY1Y2s_part1) - meanY1 * meanY2s # cov(Y1, Y2s)
  
  # Correlation between primary and secondary outcomes
  corrY1Y2s = covY1Y2s / sqrt(varY1 * varY2s)
  
  # Parameter of interest
  beta_1.true = beta_1.16
  ...
  for(nrep in 1:Nreplic){
    ...
    Y1              = Y1[,1] # Primary outcome, Y_1^(16)
    ...
    est.jointReg    = JointReg(Y1 = Y1, Y2 = Y2s, T = T, Wcat = W) # Estimation using the joint approach, when the observed covariates are included in the estimating equations.
    est.jointNC     = JointNC(Y1 = Y1, Y2 = Y2s, T = T) # Estimation using the joint approach, when the observed covariates are neglected.
    est.jointMH     = JointStratificationMH(Y1, Y2s, T = T, W = W) # Estimation of the treatment effect with the joint standardized approach with M-H weights.
    est.MH          = StratificationMH(Y1 = Y1, T = T, W = W) # Estimation of the treatment effect with the usual standardization, using MH weights. Observed covariates are used, but not the non-targeted HPV types.
    est.SSjoint     = SSJoint(Y1, Y2s, T, W) # Estimation using the stratum-specific joint approach
    ...
    res = rbind(res, cbind(recap, n = n, pY1 = paste(pY1, collapse = "_"), pY2 = paste(pY2, collapse = "_"), A = paste(a, collapse = "_"), beta_1.true = beta_1.true, corrY1Y2s = corrY1Y2s, varA = varA, beta_1.hat.modgee = est.jointReg$beta_1.hat.naive))
  }
  
  myfile  = paste0("RES_NonRandomized_LargeStrataW_SingleTargetedType-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1.true, digits = 3), collapse = "_"), ".RData")
...
for(p in 1: nrow(PARAM)){
  ...
  load(paste0("RES_NonRandomized_LargeStrataW_SingleTargetedType-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData"))
  RES = rbind(RES, res)
}
...
RECAP$corrY1Y2s = as.numeric(RECAP$corrY1Y2s)
...
myfile  = paste0("RECAP_NonRandomized_LargeStrataW_SingleTargetedType-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")
...
grid.draw(z)
# Save the plot
pdf(paste0("Comparison_NonRandomized_LargeStrataW_SingleTargetedType-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".pdf"),  width = 10, height = 7)
...
myfile  = paste0("Eff_NonRandomized_LargeStrataW_SingleTargetedType-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")  # save as Rdata
