...
# The objective is to assess the performance of our proposed approaches (Joint-Reg, Joint-MH), compared to more naive approaches (e.g MH).
...
load("Parameters.RData") # files with certain of the fixed parameters
library(parallel)
...
# Sample sizes
N     = c(5 * 10^4)    # simulations with n = 5 * 10^4 and 10^4  are performed in Simul_NonRandomized_Part1.R                                         
Part = c(1,2) # Breaking down the number of replications in 2 so we can parallelize more for the larger sample size

k = 1
PARAM = matrix(nrow = length(P_Y1[,1])*length(Aa[,1])*length(N)*length(Part), ncol = length(P_Y1[1,]) + length(Aa[1,]) + 2) 
for(l in 1:length(P_Y1[,1])){
  for(m in 1:length(Aa[,1])){
    for(i in 1:length(N)){
      for(j in 1:length(Part)){
        PARAM[k,] = c(P_Y1[l,], Aa[m,], N[i], Part[j])
        k = k+1
      }}}}
...
Onerun = function(p){
  ...
  myfile  = paste0("RES_NonRandomized-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), "Part", part, ".RData")
...
for(p in 1: nrow(PARAM)){
  ...
  if(n == (5 * 10^4)){
    load(paste0("RES_NonRandomized-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), "Part", 1, ".RData"))
    RES = rbind(RES, res)
    load(paste0("RES_NonRandomized-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), "Part", 2, ".RData"))
    RES = rbind(RES, res)
  }else{
    load(paste0("RES_NonRandomized-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData"))
    RES = rbind(RES, res)
  }
}
...
myfile  = paste0("RECAP_NonRandomized-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")
...
grid.draw(z)
# Save the plot
pdf(paste0("Comparison_NonRandomized-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".pdf"),  width = 10, height = 7)
...
beta_1          = c(beta_1.16, beta_1.18)
myfile  = paste0("Eff_NonRandomized-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")  # save as Rdata
