...
# Part 3, for beta2j = nu_3.
# Code for beta2j = nu_1 and nu_2 are given in Simul_Randomized_TreatmentEffectY2_Part1_revision.R and Simul_Randomized_TreatmentEffectY2_Part2_revision.R
...
beta_2          = rep(-0.064, 20)  # We assume that the vaccine has an effect on HPV viruses that are not targeted by the vaccine.
# The beta2j are taken from the CVT (with incident + prevalent infections); we used the CI bound on the vaccine effect on Y2 that is the farthest from 0.
...
Onerun = function(p){
  ...
  myfile  = paste0("RES_NonRandomized_TreatmentEffectY2_Part3-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")
...
for(p in 1: nrow(PARAM)){
  ...
  load(paste0("RES_NonRandomized_TreatmentEffectY2_Part3-n", n, "-pY1", paste(pY1, collapse = "_"), "-A", paste(a, collapse = "_"), "-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData"))
  RES = rbind(RES, res)
}
...
myfile  = paste0("RECAP_NonRandomized_TreatmentEffectY2_Part3-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")
...
grid.draw(z)
# Save the plot
pdf(paste0("Comparison_NonRandomized_TreatmentEffectY2_Part3-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".pdf"),  width = 10, height = 7)
...
beta_1          = c(beta_1.16, beta_1.18)
myfile  = paste0("Eff_NonRandomized_TreatmentEffectY2_Part3-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".RData")  # save as Rdata
...
write.csv(Eff, file = paste0("Eff_NonRandomized_TreatmentEffectY2_Part3-beta1", paste(round(beta_1, digits = 3), collapse = "_"), ".csv")) # save as csv
